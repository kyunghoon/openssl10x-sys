...
impl Build {
    pub fn new() -> Build {
        assert!(cfg!(feature = "wsl2-workaround"));
        Build {
            out_dir: if cfg!(feature = "wsl2-workaround") {
                Some(PathBuf::from("/tmp/openssl-build"))
            } else {
                env::var_os("OUT_DIR").map(|s| PathBuf::from(s).join("openssl-build"))
            },
    ...
    pub fn build(&mut self) -> Artifacts {
        ...
        if target.contains("msvc") {
            ...
            if cfg!(feature = "wsl2-workaround") {
                let mut install = self.cmd_make();
                install.arg("build_apps").current_dir(&inner_dir);
                self.run_command(install, "installing OpenSSL");

                let mut install = self.cmd_make();
                install.arg("install_sw").current_dir(&inner_dir); // software only (no docs)
                self.run_command(install, "installing OpenSSL");
            } else {
                let mut install = self.cmd_make();
                install.arg("install_dev").current_dir(&inner_dir);
        ...
        if cfg!(feature = "wsl2-workaround") {
            let orig_install_dir = env::var_os("OUT_DIR").map(|s| PathBuf::from(s).join("openssl-build").join("install")).unwrap();
            ["bin", "include", "lib", "ssl"].iter().for_each(|dirname| {
                fs::create_dir_all(&orig_install_dir.join(dirname)).unwrap();
                cp_r(&install_dir.join(dirname), &orig_install_dir.join(dirname));
            });
            Artifacts {
                lib_dir: orig_install_dir.join("lib"),
                bin_dir: orig_install_dir.join("bin"),
                include_dir: orig_install_dir.join("include"),
                libs: libs,
